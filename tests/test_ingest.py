...
def test_select_reference_video_refines_when_first_round_is_bad(svc):
    initial_candidates = [
        ...
        {
            "video_id": "bad-3",
            "title": "Fast cuts carrying upstairs",
            "description": "",
            "channel_title": "Channel C",
            "query": "person carrying box upstairs",
            "url": "https://www.youtube.com/watch?v=bad-3",
        },
    ]
    refined_candidates = [
        {
        ...
        {
            "video_id": "good-3",
            "title": "Worker climbing stairs with box full body",
            "description": "",
            "channel_title": "Channel F",
            "query": "person carrying box up stairs side view",
            "url": "https://www.youtube.com/watch?v=good-3",
        },
    ]
    ...
    assert mock_search.call_args_list[0].args[0] == "person carrying box upstairs"
    assert mock_search.call_args_list[1].args[0] == "person carrying box up stairs side view"
...
def test_duration_filter_rejects_too_short(svc):
    ...
    # Falls back to unfiltered pool when everything is filtered
    assert result[0].video_id == "short-1"
...
def test_duration_filter_falls_back_when_all_filtered(svc):
    """When all candidates fail duration filter, returns unfiltered pool."""
    with patch.object(
        svc,
        "search_youtube_candidates",
        return_value=[_make_candidate("short-only")],
    ), patch.object(
        svc,
        "_fetch_video_details",
        return_value={"short-only": {"duration_seconds": 2, "view_count": 1000}},
    ):
        result = svc._collect_candidate_videos(["human walking"], shortlist_size=3)
    assert len(result) >= 1
    assert result[0].video_id == "short-only"
...
# --- Thumbnail usage in _review_video_candidates ---

def test_review_uses_thumbnail_inline_data_not_file_data(svc):
    os.environ["GEMINI_API_KEY"] = "fake"
    svc._gemini_client = MagicMock()
    mock_types = MagicMock()
    svc._genai_types = mock_types

    fake_thumb = b"\xff\xd8\xff"  # minimal JPEG magic bytes
    mock_resp = MagicMock()
    mock_resp.text = (
    ...
    from demo.services.ingest_service import SearchCandidate
    ...
    with patch("demo.services.ingest_service._fetch_thumbnail_bytes", return_value=fake_thumb), \
         patch.object(svc._gemini_client.models, "generate_content", return_value=mock_resp):
        svc._review_video_candidates("make robot walk", {"task_goal": "walk"}, candidates)

    # Blob should have been constructed with the thumbnail bytes
    mock_types.Blob.assert_called_once_with(data=fake_thumb, mime_type="image/jpeg")
    # FileData must NOT have been used
    mock_types.FileData.assert_not_called()


def test_review_skips_thumbnail_when_fetch_returns_none(svc):
    os.environ["GEMINI_API_KEY"] = "fake"
    svc._gemini_client = MagicMock()
    mock_types = MagicMock()
    svc._genai_types = mock_types
    ...
    with patch("demo.services.ingest_service._fetch_thumbnail_bytes", return_value=None), \
         patch.object(svc._gemini_client.models, "generate_content", return_value=mock_resp):
        svc._review_video_candidates("make robot walk", {"task_goal": "walk"}, candidates)
