...
<!-- ROUTINE:BEGIN -->
**2026-09-25** · build order: still 7/8 · no new evidence since 2026-08-11 · no-op (clean firing —
...
issue every firing since 2026-08-10 has hit. This session's checkout started detached at bare
`master`, which alone shows only `g1.py`/`records.py`/`smoke.py` and one `dev-a.xml` — the same
misleading starting point as every prior near-miss. Before writing any file, ran
`git fetch origin routine/experiments` + `git log origin/routine/experiments --not origin/master`,
which surfaced that branch 60 commits ahead of `master`, carrying build-order steps 1–7 in open
draft PR #6. No duplicate code was designed or written this time — the check ran first. Checked out
`routine/experiments` (tip `ddb0241`, matching PR #6's head) and re-verified every standing blocker
fresh rather than trusting yesterday's entry:

- PR #6 — open, draft, 60 commits, base `master` unchanged (`d2e853c`). `get_reviews` → `[]`;
  `get_comments` (100/page) → 50 comments, all `author_association: OWNER` (one more than
  yesterday's 49 — that day's own self-report). No distinct human reply since the 2026-08-03
  reconciliation comment — now **53 days** (computed fresh via date arithmetic from 2026-08-03).
...
(two prior notifications, 2026-08-10 and 2026-08-25) and the already-escalated PR silence, now one
day longer; today's firing cost zero repo-state beyond this README/registry update.
<!-- ROUTINE:END -->
